...
$rg = Get-AzResourceGroup `
  ...
  if( $null -ne $rg )
  {
    ...
    Remove-AzResourceGroup -Name $json.Topology.ResourceGroup -Force
    LogMsg "Complete."
  }
...
$sg = Get-AzNetworkSecurityGroup `
 ...
 if( $null -ne $sg )
 {
   ...
   New-AzNetworkSecurityGroup -Name $json.Topology.NetworkSecurityGroup.Name `
    -ResourceGroupName $json.Topology.ResourceGroup `
    -Location $json.Topology.Location `
    -SecurityRules $rules
   $sg = Get-AzNetworkSecurityGroup `
   ...
   $vnet = Get-AzVirtualNetwork `
    ...
    if ($null -eq $vnet) {
        ...
        $vsubnet = New-AzVirtualNetworkSubnetConfig -Name $json.Topology.Network.Subnet.Name `
          -AddressPrefix $json.Topology.Network.Subnet.AddressPrefix -NetworkSecurityGroup $sg
        New-AzVirtualNetwork  -Name $json.Topology.Network.Name `
          ...
          $vnet = Get-AzVirtualNetwork -Name $json.Topology.Network.Name `
            -ResourceGroupName $json.Topology.ResourceGroup
    }
 }
...
foreach( $def in $json.Topology.VirtualMachine )
{
  ...
  $vm = New-AzVMConfig -VMName $def.Name -VMSize $def.VMSize
  ...
  $nic = New-AzNetworkInterface -Name $def.NIC.Name -ResourceGroupName $json.Topology.ResourceGroup `
    -Location $json.Topology.Location `
  ...
  Set-AzVMOSDisk -VM $vm -Name $def.Name `
  ...
  $NewVM = New-AzVM -ResourceGroupName $json.Topology.ResourceGroup `
