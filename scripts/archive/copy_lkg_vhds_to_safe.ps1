...
if ($excludePackages -eq $false) {
    ...
    Get-AzStorageBlob -Container $destPkgContainer -blob * | ForEach-Object {Remove-AzStorageBlob -Blob $_.Name -Container $destPkgContainer}
    if ($? -eq $false) {
        $failure_point="ClearingContainers"
        ErrOut($failure_point)
    }
    ...
    Set-AzCurrentStorageAccount -ResourceGroupName $sourceRG -StorageAccountName $sourceSA > $null

    write-verbose "Copying the VHDs to LKG"
    $blobs=Get-AzStorageBlob -Container $sourcePkgContainer -Blob *
    foreach ($oneblob in $blobs) {
        ...
        $blob = Start-AzStorageBlobCopy -SrcBlob $sourceName -SrcContainer $sourcePkgContainer  -DestContainer $destPkgContainer -DestBlob $targetName `
...
Get-AzStorageBlob -Container $sourceContainer -Blob "*-BORG.vhd"
$blobs=Get-AzStorageBlob -Container $sourceContainer -Blob "*-BORG.vhd"
foreach ($oneblob in $blobs) {
    $sourceName=$oneblob.Name
    ...
    Start-AzStorageBlobCopy -SrcBlob $sourceName -DestContainer $destContainer -SrcContainer $sourceContainer -DestBlob $targetName -Context $sourceContext -DestContext $destContext -Force > $null
    if ($? -eq $true) {
        $copyblobs.Add($targetName)
    } else {
